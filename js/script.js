...
const translations = {
  fa: {
    ...
    videoBtnTg: "ارسال به تلگرام",
    videoBtnBale: "ارسال به بله",
    videoLabelTg: "ارسال به تلگرام",
    videoLabelBale: "ارسال به بله",
    copyBtn: "کپی",
    copied: "کپی شد ✓",
    labelName: "نام شما",
    phName: "مثلاً: امیر",
  ...
  en: {
    ...
    videoBtnTg: "Send on Telegram",
    videoBtnBale: "Send on Bale",
    videoLabelTg: "Send on Telegram",
    videoLabelBale: "Send on Bale",
    copyBtn: "Copy",
    copied: "Copied ✓",
    labelName: "Your Name",
    phName: "e.g. Amir",
...
// ---- video submission glass bars (Telegram + Bale) ----
const videoBtnTg = document.getElementById('videoBtnTg');
const videoBtnBale = document.getElementById('videoBtnBale');
const glassBarTg = document.getElementById('glassBarTg');
const glassBarBale = document.getElementById('glassBarBale');
const copyBtnTg = document.getElementById('copyHandleTg');
const copyBtnBale = document.getElementById('copyHandleBale');

videoBtnTg.addEventListener('click', () => {
  glassBarTg.classList.toggle('show');
  videoBtnTg.classList.toggle('active', glassBarTg.classList.contains('show'));
  glassBarBale.classList.remove('show');
  videoBtnBale.classList.remove('active');
});

videoBtnBale.addEventListener('click', () => {
  glassBarBale.classList.toggle('show');
  videoBtnBale.classList.toggle('active', glassBarBale.classList.contains('show'));
  glassBarTg.classList.remove('show');
  videoBtnTg.classList.remove('active');
});

copyBtnTg.addEventListener('click', () => {
  navigator.clipboard.writeText('@IamRITM').then(() => {
    const dict = translations[currentLang];
    copyBtnTg.textContent = dict.copied;
    setTimeout(() => { copyBtnTg.textContent = dict.copyBtn; }, 1800);
  });
});

copyBtnBale.addEventListener('click', () => {
  navigator.clipboard.writeText('@TheRITM2').then(() => {
    const dict = translations[currentLang];
    copyBtnBale.textContent = dict.copied;
    setTimeout(() => { copyBtnBale.textContent = dict.copyBtn; }, 1800);
  });
});
